...
def validate_config(config: "TrainingConfig") -> Tuple[bool, List[str]]:
    ...
    if config.source_dim % 8 != 0:
        errors.append(
            f"source_dim ({config.source_dim}) must be divisible by 8 for efficient computation"
        )

    if config.target_dim % 8 != 0:
        errors.append(
            f"target_dim ({config.target_dim}) must be divisible by 8 for efficient computation"
        )

    if config.predictor_dim % 8 != 0:
        errors.append(
            f"predictor_dim ({config.predictor_dim}) must be divisible by 8 for efficient computation"
        )
    ...
    if config.batch_size > 128:
        errors.append(
            f"batch_size ({config.batch_size}) is very large and may cause OOM errors"
        )
    ...
    if cuda_available:
        ...
        estimated_memory = (
            config.source_dim * config.source_depth * config.batch_size
        ) / 1e6

        if estimated_memory > gpu_memory * 0.8:
            errors.append(
                f"WARNING: Estimated memory usage ({estimated_memory:.1f}GB) may exceed GPU memory ({gpu_memory:.1f}GB)"
            )
